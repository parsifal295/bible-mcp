...
def test_validate_source_reference_accepts_existing_single_verse(temp_dir) -> None:
    source_path = temp_dir / "source.sqlite"
    conn = sqlite3.connect(source_path)
    conn.execute(
        """
    ...
    validate_source_reference(
        SourceBibleConfig(path=source_path, table="verses"),
        "Genesis 12:1",
    )


def test_validate_source_reference_raises_for_missing_verse(temp_dir) -> None:
    source_path = temp_dir / "source.sqlite"
    conn = sqlite3.connect(source_path)
    conn.execute(
        """
